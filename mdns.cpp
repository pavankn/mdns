...
#define MAX_FOUND_IPS 16

typedef struct {
	struct sockaddr_in addrs[MAX_FOUND_IPS];
	size_t count;
} mdns_ip_list_t;
...
int
is_ip_in_list(mdns_ip_list_t* list, struct sockaddr_in* addr) {
	for (size_t i = 0; i < list->count; ++i) {
		if (list->addrs[i].sin_addr.s_addr == addr->sin_addr.s_addr) {
			return 1;
		}
	}
	return 0;
...
void
add_ip_if_new(mdns_ip_list_t* list, struct sockaddr_in* addr) {
	if (list->count >= MAX_FOUND_IPS)
		return;
	if (!is_ip_in_list(list, addr)) {
               ...
               size_t record_length, void* user_data) 
{
	...
	mdns_ip_list_t* ip_list = (mdns_ip_list_t*)user_data;
	...
	if (from->sa_family == AF_INET) {
		struct sockaddr_in* from_ipv4 = (struct sockaddr_in*)from;
		add_ip_if_new(ip_list, from_ipv4);
	}
	return 0;
}
...
service_callback(int sock, const struct sockaddr* from, size_t addrlen, mdns_entry_type_t entry,
	...
	if ((name.length == (sizeof(dns_sd) - 1)) &&
		...
		if ((rtype == MDNS_RECORDTYPE_PTR) || (rtype == MDNS_RECORDTYPE_ANY)) {
			...
			answer.name = name;  // The DNS-SD domain name
			answer.type = MDNS_RECORDTYPE_PTR;  // The record type is PTR
			answer.data.ptr.name = service->service;  // The service name we advertise
			...
			if (unicast) {
				mdns_query_answer_unicast(sock, from, addrlen, sendbuffer, sizeof(sendbuffer),
				                          ...
				                          name.str, name.length, answer, 0, 0, 0,
				                          0);
			} else {
...
// Send a DNS-SD query
static int
send_dns_sd(mdns_ip_list_t* found_ips) {
	int sockets[32];
...
service_mdns(const char* hostname, const char* service_name, int service_port) {
	...
	service_string.length = static_cast<size_t>(strlen(service_name));

	mdns_string_t hostname_string;
	hostname_string.str = hostname;
	hostname_string.length = static_cast<size_t>(strlen(hostname));
	...
	service_instance_string.length = static_cast<size_t>(strlen(service_instance_buffer));
	...
	hostname_qualified_string.length = static_cast<size_t>(strlen(qualified_hostname_buffer));
	...
	// A record mapping "<hostname>.local." to IPv4 address
	service.record_a = {};
	...
	// AAAA record mapping "<hostname>.local." to IPv6 address
	service.record_aaaa = {};
	...
	service.txt_record[0].data.txt.key.str = "test";
	service.txt_record[0].data.txt.key.length = 4;
	service.txt_record[0].data.txt.value.str = "1";
	service.txt_record[0].data.txt.value.length = 1;
	...
	service.txt_record[1].data.txt.key.str = "other";
	service.txt_record[1].data.txt.key.length = 5;
	service.txt_record[1].data.txt.value.str = "value";
	service.txt_record[1].data.txt.value.length = 5;
...
mdns_discover_ips(char** out_ips, int max_ips) {
	...
	mdns_ip_list_t found_ips = {0};

	send_dns_sd(&found_ips);

	int count = 0;
	for (size_t i = 0; i < found_ips.count && count < max_ips; ++i) {
		char ip_str[INET_ADDRSTRLEN] = {0};
		InetNtopA(AF_INET, &found_ips.addrs[i].sin_addr, ip_str, INET_ADDRSTRLEN);
		printf("Discovered IP: %s\n", ip_str);
		out_ips[count] = _strdup(ip_str);  // caller must free
		++count;
	}
